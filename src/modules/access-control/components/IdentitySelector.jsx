...
import { getInitials } from "../data/helper";
...
function IdentitySelector({ companyId, selectedIdentity, onSelect }) {
  ...
  const fetchRoles = async (text = "") => {
    const version = ++requestVersion.current;
    try {
      setLoading(true);
      const res = await makeRequest('/permissions/roles', {
        method: "POST",
      ...
      if (!res?.success) {
        toast.error(res?.message || "Error while fetching roles");
        setIdentities([]);
        onSelect?.(null);
        return;
      }

      const nextIdentities = (res.data || []).map(role => ({...role, id: role.roleID, name: role.roleName, initials: getInitials(role.roleName), badge: 'ROLE', email: role.user_count + ' users', companyLabel: role.company_name})).filter(role => role.name.toLowerCase().includes(text.toLowerCase()) || role.companyLabel?.toLowerCase().includes(text.toLowerCase()));
      setIdentities(nextIdentities);
    } catch (error) {
      toast.error(error.message || "Error while fetching roles");
      setIdentities([]);
      onSelect?.(null);
    } finally {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      fetchRoles(searchText.trim());
    }, searchText.trim() ? 350 : 0);

    return () => { window.clearTimeout(timeout); requestVersion.current++; };
  }, [companyId, searchText]);

  const countLabel = useMemo(() => {
    if (loading) return "Loading...";
    return `${identities.length} role${identities.length === 1 ? "" : "s"}`;
  ...
  return (
    <aside className="min-h-full border border-slate-200 bg-white">
      <div className="flex items-center justify-between gap-2 border-b border-slate-200 px-4 py-3">
        <div>
          <h3 className="text-sm font-semibold text-slate-700">Select Role</h3>
        ...
        <button
          ...
          onClick={() => fetchRoles(searchText.trim())}
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="max-h-[calc(100vh-250px)] space-y-2 overflow-y-auto pr-1">
          ...
          {!loading && identities.map((identity) => (
            <button
              type="button"
              key={`${identity.company_id}:${identity.id}`}
              className={`flex w-full items-center gap-3 rounded-md border px-3 py-2 text-left transition-colors ${selectedIdentity?.id === identity.id && selectedIdentity?.company_id === identity.company_id
