...
function MenuRow({
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, }} className={`w-[100%] grid min-w-225 grid-cols-[auto_minmax(220px,1.4fr)_minmax(150px,0.8fr)_minmax(180px,1fr)_90px_auto] items-center gap-3 border border-slate-200 bg-white px-2 py-2 shadow-sm ${isDragging ? "opacity-60 ring-2 ring-indigo-200" : ""}`} >
      <button
        type="button"
        {...(canSort ? attributes : {})}
        {...(canSort ? listeners : {})}
...
function MenuList({
  ...
  if (!rows.length) {
    return (
      <div className="flex flex-col min-h-[360px] w-[100%] items-center justify-center rounded-md bg-white">
        <img src={import.meta.env.BASE_URL + "svg/no_data.svg"} alt="" className="h-[40%] w-[40%]" />
        <div className="text-center">
          <div className="text-md font-semibold text-slate-800">No menus found</div>
          <div className="mt-1 text-sm text-slate-500">Create a menu to show it here.</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={rows.map((menu) => getMenuId(menu))} strategy={verticalListSortingStrategy}>
        <div className="flex flex-col min-h-[360px] justify-center items-center w-[100%] space-y-2 overflow-x-auto" style={{ "scrollbarWidth": "none" }}>
          {rows.map((menu) => (
            <MenuRow
              key={getMenuId(menu)}
              menu={menu}
