...
import { doctorsFallbackColumns, doctorsModuleSchema } from "../data/module.schema";

export const useDoctorsTableConfig = ({ resolvedMenuID, filterState }) => {
    ...
    const columnOptions = {
        skipFields: doctorsModuleSchema.skipFields,
        columnMappings: doctorsModuleSchema.columnMappings,
        tableCellConfig: doctorsModuleSchema.tableCellConfig,
        filterFieldOptions: doctorsModuleSchema.filterFieldOptions,
    };

    const resolvedColumns = useMemo(
        () => buildTableColumnsFromStructure(fields, doctorsFallbackColumns, columnOptions),
        [fields]
    );

    const defaultVisibleColumnKeys = useMemo(
        () => doctorsFallbackColumns.map((column) => column.key),
        []
    );
    ...
    const resolvedFilterFields = useMemo(() =>
        buildFilterFieldsFromStructure(
            fields,
            doctorsModuleSchema.defaultColumns.map((key) => ({
                label:
                    doctorsFallbackColumns.find((column) => column.key === key)?.label || key,
                value: key,
                type: "text",
            })),
            columnOptions
